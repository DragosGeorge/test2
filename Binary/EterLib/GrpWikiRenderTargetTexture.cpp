...
bool CGraphicWikiRenderTargetTexture::Create(const _wint32 width, const _wint32 height, const D3DFORMAT texFormat, const D3DFORMAT depthFormat)
{
	Reset();
...
bool CGraphicWikiRenderTargetTexture::CreateRenderTexture(const _wint32 width, const _wint32 height, const D3DFORMAT format)
{
	m_d3dFormat = format;
...
bool CGraphicWikiRenderTargetTexture::CreateRenderDepthStencil(const _wint32 width, const _wint32 height, const D3DFORMAT format)
{
...
{
	const _wfloat sx = static_cast<_wfloat>(m_renderRect.left) - 0.5f + static_cast<_wfloat>(m_renderBox.left);
	const _wfloat sy = static_cast<_wfloat>(m_renderRect.top) - 0.5f + static_cast<_wfloat>(m_renderBox.top);
	const _wfloat ex = static_cast<_wfloat>(m_renderRect.left) + (static_cast<_wfloat>(m_renderRect.right) - static_cast<_wfloat>(m_renderRect.left)) - 0.5f - static_cast<_wfloat>(m_renderBox.right);
	const _wfloat ey = static_cast<_wfloat>(m_renderRect.top) + (static_cast<_wfloat>(m_renderRect.bottom) - static_cast<_wfloat>(m_renderRect.top)) - 0.5f - static_cast<_wfloat>(m_renderBox.bottom);
	
	const _wfloat texReverseWidth = 1.0f / (static_cast<_wfloat>(m_renderRect.right) - static_cast<_wfloat>(m_renderRect.left));
	const _wfloat texReverseHeight = 1.0f / (static_cast<_wfloat>(m_renderRect.bottom) - static_cast<_wfloat>(m_renderRect.top));
	
	const _wfloat su = m_renderBox.left * texReverseWidth;
	const _wfloat sv = m_renderBox.top * texReverseHeight;
	const _wfloat eu = ((m_renderRect.right - m_renderRect.left) - m_renderBox.right) * texReverseWidth;
	const _wfloat ev = ((m_renderRect.bottom - m_renderRect.top) - m_renderBox.bottom) * texReverseHeight;
